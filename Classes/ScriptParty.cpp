...
int addGold(lua_State* pL)
{
	...
	GameScene::getInstance()->addGold(gold);
...
{
	int gold = (int)luaL_checkinteger(pL, 1);
	bool ret = GameScene::getInstance()->subGold(gold);
...
int subGood(lua_State* pL)
{
	...
	bool ret = GameScene::getInstance()->subGood(name, number);
...
int subEquipment(lua_State* pL)
{
	...
	bool ret = GameScene::getInstance()->subEquipment(playerName, goodName, number);
...
int recover(lua_State* pL)
{
	...
	//恢复血量
	auto& list = playerLayer->getCharacterList();
	/*当id为-1时，表示全队进行恢复血量
