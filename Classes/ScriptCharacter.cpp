...
int showAnimation(lua_State* pL)
{
	...
	else if (lua_gettop(pL) == 3)
	{
		...
		animation = StaticData::getInstance()->getWalkingAnimation(chartletName, dir);
	}
	//运行动画
...
int moveToward(lua_State* pL)
{
	...
	float duration = 0.f;
	//角色存在并且不为空。进行移动
	if (character != nullptr && gameScene->isPassing(toTile))
	{
		//移动
		duration = character->moveToward(toTile);
		bRet = true;
	}
...
int setDurationPerGrid(lua_State* pL)
{
	...
	character->setDurationPerGrid(duration);
