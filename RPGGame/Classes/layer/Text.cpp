...
float Text::wind(const string& text, float delay)
{
	if (text.empty())
		return 0.f;
	...
	if (delay < 0.f)
		delay = 0.f;
	bool visible = delay > 0.f ? false : true;
	//总持续时间
	float duration = 0.f;
	//TODO
	float scale = 1.f;
	...
	for (unsigned int i = 0;i < stringLen;)
	{
		...
		else
		{
			...
			//取得字相关精灵
			fontChar = SpritePool::getInstance()->createObject();
		...
		fontChar->setVisible(visible);
		this->addChild(fontChar);
		//保存
