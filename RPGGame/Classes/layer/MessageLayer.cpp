...
float MessageLayer::showText(const string& name, const string& text, TextPosition textPos, bool run, const string& faceFilename, int index, FacePosition facePos)
{
	...
	auto spriteFrame = StaticData::getInstance()->getCharacterData()->getFaceSpriteFrame(faceFilename, index);
...
void MessageLayer::clickBtn(Object* sender)
{
	...
	auto luaStack = scriptManager->getLuaStack();

	luaStack->pushString(text.c_str());
	scriptManager->resumeCoroutine(WaitType::Button, 1);
}